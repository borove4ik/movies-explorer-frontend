...
const BurgerMenu = ({ opened, closeMenu }) => {
  ...
  const goHome =()=>{navigate('/'); closeMenu()}
  const toMovies =()=>{navigate('/movies'); 
  closeMenu()
}
  const toSavedMovies =()=>{navigate('/saved-movies'); closeMenu()}
  const toProfile =()=>{navigate('/profile'); closeMenu()}
    return (
      ...
      <div className={burgerStyles}>
        <div className="burger-menu">
          ...
          <div className="burger-menu__navigate-wrapper">
            ...
            <button className="burger-menu__navigation-button" type="button" onClick={toSavedMovies}>Сохранённые фильмы</button>
          </div>
